...
{
    const MAIL_REMINDER_VERIFY = 1;
    const MAIL_PHOTO_REMOVED = 2;
    const MAIL_MESSAGE_SENT = 3;
    public static $type;
	/**
...
// 		$compiled_template = self::compileMultipartTemplate($template, $template_partials);
		...
		foreach ($to as $receiver) {
			switch ($action) {
				case 'send':
					$statuses[$receiver] = Mailer::send ( $receiver,  $email->subject, $body);
					break;
				case 'enqueue':
	...
	private static function process_bk($to, $template_name, $template_vars = array(), $template_partials = array(), $action = 'send') {
		if (is_string($to)) {
			$to = array($to);
		}
		/** @var $template SimpleMailerTemplate */
