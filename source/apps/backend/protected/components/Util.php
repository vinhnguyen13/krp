...
class Util {
	...
	public static function generateOnlyNumber($length = 8) {
		$salt = "0123456789";
		$len = strlen ( $salt );
		$makepass = '';
	...
	public static function writeCookie2($data, $cookietimeout = -1) {
		$value = json_encode ( $data ['value'] );
		//$value = self::encrypt ( $value ) . '|' . time () . '|' . md5 ( $value . time () );
		$Cookie = new CHttpCookie ( $data ['name'], $data ['valueDefault'] );
		$Cookie->value = $value;
	...
	public static function getCookie2($name) {
		$cookie = Yii::app ()->request->cookies [$name];
		if(isset($cookie->value)) {
			return $cookie->value;
		}
	...
	public static function clearCookie2($name) {
    ...
    public static function setSSOCookie($name, $value, $time = 3600) {
    	$cookie = new CHttpCookie($name, $value);
    	$cookie->expire = time()+$time;
    	// $Cookie->path = Yii::app ()->params->cookie ['path'];
    	$cookie->domain = Yii::app ()->id;
    	Yii::app()->request->cookies[$name] = $cookie;
    }
    ...
    public static function stripOutHTML($des){
        if(!empty($des)){
